...
export default function App() {
  ...
  const handleAdminSceneSelect = (scene: Scene) => {
    ...
    fetch(`/api/deficits/${scene.id}`)
      .then(res => res.json())
      .then(data => setAdminDeficits(data));
  };

  const saveDeficit = () => {
    const method = editingDeficit.id ? "PUT" : "POST";
    const url = editingDeficit.id ? `/api/admin/deficits/${editingDeficit.id}` : "/api/admin/deficits";
    
    fetch(url, {
      method,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ...editingDeficit, sceneId: adminSelectedScene?.id })
    })
    .then(res => res.json())
    .then(() => {
  ...
  const handleDeficitFound = (deficit: Deficit) => {
    if (!foundDeficits.includes(deficit.id)) {
      ...
      setSelectedDeficit(deficit);
      setAssessmentStep(3); // Show feedback directly if already found
    }
  };

  const submitAssessmentStep = (value: any) => {
    const steps = ["wichtigkeit", "abweichung", "unfallschwere"];
    ...
    if (assessmentStep < 2) {
      ...
      // Final step: Calculate points and show feedback
  ...
  const calculatePoints = (assessment: any) => {
    if (!selectedDeficit) return;
    let points = 100; // Base points for finding it
    ...
    if (
      ...
      points += 100; // Bonus for fully correct assessment
    }
              ...
              <nav className="flex items-center gap-4 border-l border-white/10 pl-6">
                <button 
                  ...
                  className={cn(
                    ...
                    view === "dashboard" ? "text-blue-400" : "text-white/60"
                  )}
                >
                ...
                <button 
                  ...
                  className={cn(
                    "flex items-center gap-2 text-sm font-medium transition-colors hover:text-blue-400",
                    view === "leaderboard" ? "text-blue-400" : "text-white/60"
                  )}
                >
                  <BarChart3 size={18} />
                  Ranking
                ...
                <button 
                  ...
                  className={cn(
                    "flex items-center gap-2 text-sm font-medium transition-colors hover:text-blue-400",
                    view === "admin" ? "text-blue-400" : "text-white/60"
                  )}
                >
                  <Settings size={18} />
                  Admin
      ...
      <main className="flex-1 relative overflow-auto">
        <AnimatePresence mode="wait">
          {view === "onboarding" && (
            ...
            >
              {/* Animated 360 Background Element */}
              <div className="absolute inset-0 z-0 opacity-20 pointer-events-none">
                <motion.div 
                  ...
                  className={cn(
                    "w-[200%] h-[200%] absolute top-[-50%] left-[-50%] transition-colors duration-1000",
                    theme === "light" 
                      ? "bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-blue-100 via-transparent to-transparent"
                      : "bg-[radial-gradient(circle_at_center,_var(--tw-gradient-stops))] from-blue-900/20 via-transparent to-transparent"
                  )}
                  style={{
                    backgroundImage: `url('https://picsum.photos/seed/road360/2000/1000')`,
              ...
              <div className="max-w-6xl w-full px-10 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center z-10">
                <div className="space-y-12">
                  ...
                  >
                    ...
                    <p className={cn(
                      "text-xl max-w-md leading-relaxed font-medium transition-colors duration-700",
                  ...
                  >
                    <div className="flex items-center gap-6 group cursor-default">
                      <div className={cn(
                        ...
                        theme === "light" 
                          ? "border-black/5 group-hover:bg-black group-hover:text-white" 
                          : "border-white/5 group-hover:bg-white group-hover:text-black"
                      )}>
                        <Eye size={20} />
                      </div>
                      <span className={cn(
                        "text-sm font-bold tracking-widest uppercase transition-colors duration-700",
                  ...
                  <motion.div 
                    ...
                    transition={{ delay: 1 }}
                    className={cn(
                      "pt-10 border-t transition-colors duration-700",
                  ...
                  >
                    <button 
                      onClick={() => setView("admin")}
                      className={cn(
                        "group flex items-center gap-4 text-[11px] font-bold uppercase tracking-[0.2em] transition-all",
                        theme === "light" ? "text-black/20 hover:text-black" : "text-white/20 hover:text-white"
                      )}
                    >
                ...
                <motion.div 
                  ...
                  className={cn(
                    "border rounded-[40px] p-12 transition-all duration-700",
                    theme === "light" 
                      ? "bg-white border-black/5 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.05)]" 
                      : "bg-white/5 border-white/5 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.5)]"
                  )}
                >
                    <div className="space-y-10">
                      <div className="flex justify-between items-start">
                        <div>
                          ...
                          )}>Identifizieren Sie sich für das Ranking.</p>
                        </div>
                        <button 
                          ...
                          className={cn(
                            ...
                            theme === "light" 
                              ? "border-black/5 bg-black/5 text-black hover:bg-black hover:text-white" 
                              : "border-white/5 bg-white/5 text-white hover:bg-white hover:text-black"
                          )}
                        >
                    ...
                    <div className="space-y-6">
                      <div className="relative">
                        <input 
                          ...
                          className={cn(
                            "w-full bg-transparent border-b-2 py-4 text-xl font-bold focus:outline-none transition-colors placeholder:text-black/10",
                            theme === "light" ? "border-black/5 focus:border-black" : "border-white/5 focus:border-white"
                          )}
                        />
                      </div>
                      
                      <div className={cn(
                        "flex items-center gap-4 py-4 px-6 rounded-2xl border transition-colors duration-700",
                        theme === "light" ? "bg-[#f8f9fa] border-black/5" : "bg-white/5 border-white/5"
                      )}>
                      ...
                      <button 
                        ...
                        className={cn(
                          "w-full py-6 rounded-2xl font-bold transition-all hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-3 text-lg",
              ...
              <motion.div 
                ...
                className={cn(
                  "absolute bottom-10 left-1/2 -translate-x-1/2 transition-colors duration-700",
                  theme === "light" ? "text-black/20" : "text-white/20"
                )}
              >
                      ...
                      >
                        ...
                        <div className="p-6">
                          ...
                          <button 
                            ...
                            className="w-full py-3 bg-blue-600 hover:bg-blue-500 rounded-xl font-bold text-sm transition-colors flex items-center justify-center gap-2"
                          >
            ...
            >
                        ...
                        <div className="space-y-8">
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            {Object.entries(currentAssessment).map(([key, val]) => {
                              const isCorrect = val === (selectedDeficit.correctAssessment as any)[key];
                              return (
                          ...
                          <section className="bg-white/5 rounded-2xl p-5 border border-white/5">
                            <h5 className="text-xs font-bold uppercase tracking-widest text-blue-400 mb-3 flex items-center gap-2">
              ...
              {foundDeficits.length === deficits.length && foundDeficits.length > 0 && (
                <motion.div 
                  initial={{ opacity: 0 }}
                  animate={{ opacity: 1 }}
                  className="absolute inset-0 z-[100] bg-blue-600/90 backdrop-blur-md flex items-center justify-center p-6 text-center"
                >
                  <motion.div 
                    ...
                    className="max-w-md"
                  >
                    <div className="w-24 h-24 bg-white rounded-full flex items-center justify-center text-blue-600 mx-auto mb-8 shadow-2xl">
                      <Trophy size={48} />
                    </div>
                    ...
                    <p className="text-white/80 mb-8 leading-relaxed">
                      Du hast alle {deficits.length} Sicherheitsdefizite in diesem Szenario erfolgreich identifiziert und bewertet.
                    </p>
                    <div className="flex flex-col gap-3">
                      <button 
                        onClick={finishTraining}
                        className="w-full py-4 bg-white text-blue-600 font-bold rounded-2xl hover:bg-white/90 transition-all shadow-xl"
                      >
                ...
                <div className="bg-white/5 border border-white/10 rounded-3xl p-6 space-y-4">
                  ...
                  {topics.map(t => (
                    <div 
                      ...
                      className={cn(
                        "p-4 border rounded-2xl flex items-center justify-between group cursor-pointer transition-all",
                        adminSelectedTopic?.id === t.id ? "bg-blue-600 border-blue-500" : "bg-black/40 border-white/10 hover:border-white/30"
                      )}
                    >
                      <div>
                    ...
                    <div className="space-y-3">
                      {scenes.map(s => (
                        <div 
                          ...
                          className={cn(
                            "p-4 border rounded-2xl flex items-center justify-between group cursor-pointer transition-all",
                            adminSelectedScene?.id === s.id ? "bg-blue-600 border-blue-500" : "bg-black/40 border-white/10 hover:border-white/30"
                          )}
                        >
                          <div className="flex items-center gap-3">
                            <img src={s.imageUrl} className="w-10 h-10 rounded-lg object-cover" referrerPolicy="no-referrer" />
                            <div>
                      ...
                      <button className="w-full py-3 border border-dashed border-white/20 rounded-xl text-[10px] font-bold uppercase tracking-widest hover:bg-white/5 transition-colors">
                        + Neue Szene
                      </button>
                    </div>
                  )}
                        ...
                        >
                          ...
                          <div className="grid grid-cols-2 gap-6">
                            <div className="space-y-4">
                              <div>
                                ...
                                <input 
                                  value={editingDeficit.title} 
                                  onChange={e => setEditingDeficit({...editingDeficit, title: e.target.value})}
                              ...
                              <div>
                                ...
                                <textarea 
                                  value={editingDeficit.description} 
                                  onChange={e => setEditingDeficit({...editingDeficit, description: e.target.value})}
                                  className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 h-24"
                                />
                              </div>
                            </div>
                            ...
                            <div className="bg-black/40 p-6 rounded-2xl border border-white/5 space-y-6">
                              ...
                              <div className="space-y-4">
                                <div>
                                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/40 block mb-2">1. Wichtigkeit</label>
                                  <div className="flex gap-2">
                                    {["klein", "mittel", "gross"].map(v => (
                                      <button 
                                        key={v}
                                        onClick={() => setEditingDeficit({
                                          ...editingDeficit, 
                                          correctAssessment: { ...editingDeficit.correctAssessment!, wichtigkeit: v as any }
                                        })}
                                        className={cn(
                                          "flex-1 py-2 rounded-lg text-[10px] font-bold uppercase transition-all",
                                          editingDeficit.correctAssessment?.wichtigkeit === v ? "bg-blue-600 text-white" : "bg-white/5 text-white/40"
                                        )}
                                      >
                                        {v}
                                      </button>
                                ...
                                <div>
                                  <label className="text-[10px] font-bold uppercase tracking-widest text-white/40 block mb-2">2. Abweichung</label>
                                  <div className="flex gap-2">
                                    {["klein", "mittel", "gross"].map(v => (
                                      <button 
                                        key={v}
                                        onClick={() => setEditingDeficit({
                                          ...editingDeficit, 
                                          correctAssessment: { ...editingDeficit.correctAssessment!, abweichung: v as any }
                                        })}
                                        className={cn(
                                          "flex-1 py-2 rounded-lg text-[10px] font-bold uppercase transition-all",
                                          editingDeficit.correctAssessment?.abweichung === v ? "bg-blue-600 text-white" : "bg-white/5 text-white/40"
                                        )}
                                      >
                                        {v}
                                      </button>
                                ...
                                <div className="p-3 bg-blue-500/10 rounded-xl border border-blue-500/20">
                                  ...
                                  <p className="font-bold text-sm capitalize">
                                    {getRelevanzSD(editingDeficit.correctAssessment?.wichtigkeit || "mittel", editingDeficit.correctAssessment?.abweichung || "mittel")}
                                  </p>
                                </div>
                                ...
                                <div>
                                  ...
                                  <div className="flex gap-2">
                                    {["leicht", "mittel", "schwer"].map(v => (
                                      <button 
                                        key={v}
                                        onClick={() => setEditingDeficit({
                                          ...editingDeficit, 
                                          correctAssessment: { ...editingDeficit.correctAssessment!, unfallschwere: v as any }
                                        })}
                                        className={cn(
                                          "flex-1 py-2 rounded-lg text-[10px] font-bold uppercase transition-all",
                                          editingDeficit.correctAssessment?.unfallschwere === v ? "bg-blue-600 text-white" : "bg-white/5 text-white/40"
                                        )}
                                      >
                                        {v}
                                      </button>
                          ...
                          <div className="grid grid-cols-2 gap-6">
                            <div>
                              ...
                              <textarea 
                                value={editingDeficit.feedback} 
                                onChange={e => setEditingDeficit({...editingDeficit, feedback: e.target.value})}
                                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 h-24"
                              />
                            </div>
                            <div>
                              ...
                              <textarea 
                                value={editingDeficit.solution} 
                                onChange={e => setEditingDeficit({...editingDeficit, solution: e.target.value})}
                                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-blue-500 h-24"
                              />
                            </div>
                          </div>
                          ...
                          <div className="flex justify-end gap-3 pt-6 border-t border-white/10">
                            <button 
                              onClick={() => setIsEditingDeficit(false)}
                              className="px-6 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-sm font-bold"
                            >
                              Abbrechen
                            </button>
                            <button 
                              onClick={saveDeficit}
                              className="px-8 py-3 bg-blue-600 hover:bg-blue-500 rounded-xl text-sm font-bold flex items-center gap-2"
                            >
