...
async function startServer() {
  ...
  // In Phase 2 ersetzen durch JSON-Dateien oder Supabase
  let topics = [
  ...
  app.put("/api/admin/deficits/:id", (req, res) => {
    const idx = deficits.findIndex(d => d.id === req.params.id);
    if (idx !== -1) {
      deficits[idx] = { ...deficits[idx], ...req.body };
      res.json(deficits[idx]);
    } else {
      res.status(404).json({ error: "Defizit nicht gefunden" });
    }
  });

  // Vite-Middleware für Entwicklung
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`RSI VR Tool läuft auf http://localhost:${PORT}`);
    console.log(`Netzwerkzugang (Meta Quest): http://[lokale-IP]:${PORT}`);
  });
}
