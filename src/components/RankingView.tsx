// Ranking-Ansicht – Tabelle mit Rang, Name, Punkte, Szenen
// Eigener Eintrag hervorgehoben (var(--zh-blau) Hintergrund)

import { useTranslation } from 'react-i18next'
import { Trophy, ArrowLeft } from 'lucide-react'
import { getRanking, type AppRankingEntry } from '../data/appData'
...
export default function RankingView({ username, onBack }: Props) {
  ...
  const [entries, setEntries] = useState<AppRankingEntry[]>([])
    ...
    >
      ...
      <div className="flex items-center justify-between mb-8">
        <h1
          ...
          style={{ fontSize: '24px', color: 'var(--zh-color-text)' }}
        >
        ...
        <table className="w-full text-left">
          <thead>
            <tr style={{ borderBottom: '1px solid var(--zh-color-border)', background: 'var(--zh-color-bg-secondary)' }}>
              {[t('ranking.rank'), t('ranking.name'), t('ranking.score'), t('ranking.scenes')].map(col => (
                <th
                  key={col}
                  style={{ padding: '12px 20px', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.14em', color: 'var(--zh-color-text-muted)' }}
                  className={col === t('ranking.score') ? 'text-right' : ''}
                >
                  {col}
                </th>
              ))}
          ...
          <tbody>
              ...
              return (
                ...
                >
                  ...
                  <td style={{ padding: '14px 20px', fontSize: '14px', fontWeight: isOwn ? 700 : 500, color: isOwn ? 'var(--zh-color-accent)' : 'var(--zh-color-text)' }}>
                    {entry.username}
                    {isOwn && (
                      <span style={{ marginLeft: '8px', fontSize: '10px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.12em', color: 'var(--zh-color-accent)', opacity: 0.7 }}>
                        (Sie)
                      </span>
                    )}
                  </td>
            ...
            {entries.length === 0 && (
              <tr>
                <td colSpan={4} style={{ padding: '32px 20px', textAlign: 'center', fontSize: '14px', color: 'var(--zh-color-text-disabled)' }}>
                  Noch keine Einträge.
                </td>
              </tr>
            )}
