...
export default function KursModal({ open, initial, topics, onClose, onSave }: Props) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.5)', backdropFilter: 'blur(4px)' }}
      ...
      <div ref={modalRef} role="dialog" aria-modal="true" style={{ width: '520px', maxHeight: '88vh', overflowY: 'auto', borderRadius: 'var(--rsi-radius-card)', border: '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-surface)', padding: '28px 32px', boxShadow: 'var(--rsi-shadow-lg)' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px' }}>
          <h3 style={{ fontSize: '18px', fontWeight: 700, color: 'var(--rsi-color-text)' }}>{t('admin.kurs_neu')}</h3>
        ...
        <Section label={t('admin.kurs_name')}>
          <input value={draft.name} onChange={e => setDraft(prev => prev ? { ...prev, name: e.target.value } : prev)}
            style={{ width: '100%', padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-color-text)', fontSize: '13px', fontFamily: 'var(--rsi-font)', boxSizing: 'border-box' }} />
        </Section>
        ...
        <Section label={`${t('admin.gueltig_von')} / ${t('admin.gueltig_bis')}`}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
            <div>
              <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--rsi-color-text-disabled)', marginBottom: '3px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{t('admin.gueltig_von')}</div>
              <input type="datetime-local"
                ...
                style={{ width: '100%', padding: '7px 10px', borderRadius: '6px', border: '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-color-text)', fontSize: '12px', fontFamily: 'var(--rsi-font)', boxSizing: 'border-box' }} />
            </div>
            <div>
              <div style={{ fontSize: '10px', fontWeight: 700, color: 'var(--rsi-color-text-disabled)', marginBottom: '3px', textTransform: 'uppercase', letterSpacing: '0.1em' }}>{t('admin.gueltig_bis')}</div>
              <input type="datetime-local"
        ...
        <Section label={t('admin.passwort')}>
          {draft.hatPasswort === true && (draft.passwort === undefined || draft.passwort === '') ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
              <span style={{ fontSize: '12px', color: 'var(--rsi-gruen)', fontWeight: 700 }}>Passwort gesetzt (gehasht)</span>
              <button
                type="button"
                onClick={() => setDraft(prev => prev ? { ...prev, passwort: null } : prev)}
                style={{ background: 'none', border: '1px solid var(--rsi-color-border)', borderRadius: '6px', padding: '4px 10px', fontSize: '11px', cursor: 'pointer', color: 'var(--rsi-rot)', fontFamily: 'var(--rsi-font)' }}
              >
            ...
            <div style={{ position: 'relative' }}>
              <input
                ...
                data-lpignore="true"
                data-1p-ignore="true"
                style={{ width: '100%', padding: '8px 40px 8px 12px', borderRadius: '6px', border: '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-color-text)', fontSize: '13px', fontFamily: 'var(--rsi-font)', boxSizing: 'border-box' }}
              />
              <button
                type="button"
                onClick={() => setShowPasswort(v => !v)}
                style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', cursor: 'pointer', color: 'var(--rsi-color-text-muted)', display: 'flex', alignItems: 'center' }}
              >
        ...
        <Section label={t('admin.kurs_code')}>
          <div style={{ display: 'flex', gap: '8px' }}>
            <input value={draft.zugangscode} onChange={e => setDraft(prev => prev ? { ...prev, zugangscode: e.target.value } : prev)}
              ...
              style={{ flex: 1, padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-color-text)', fontSize: '13px', fontFamily: 'var(--rsi-font)' }} />
            <button onClick={() => setDraft(prev => prev ? { ...prev, zugangscode: generateKursCode() } : prev)}
              style={{ padding: '8px 14px', borderRadius: '6px', border: '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-blau)', fontSize: '12px', fontWeight: 700, cursor: 'pointer', whiteSpace: 'nowrap' }}>
