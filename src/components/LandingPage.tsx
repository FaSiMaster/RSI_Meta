...
export default function LandingPage({ theme, onToggleTheme, onStart, onAdmin }: Props) {
  ...
  return (
    <div className="flex flex-col" style={{ minHeight: '100dvh', background: 'var(--rsi-color-bg)', fontFamily: 'var(--rsi-font)' }}>

      {/* ── Top-Bar ── */}
      <div className="flex items-center justify-between px-5 sm:px-8" style={{ minHeight: 'var(--rsi-navbar-h)', borderBottom: '1px solid var(--rsi-color-border)' }}>
        <div className="flex items-center gap-3">
          <span className="text-sm font-extrabold" style={{ color: 'var(--rsi-color-text)' }}>RSI VR Tool</span>
        </div>
        <div className="flex items-center gap-3">
          <LanguageSwitcher />
          <button
            ...
            style={{ width: '30px', height: '30px', borderRadius: '50%', border: '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-color-text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', flexShrink: 0 }}
          >
          ...
          <button
            ...
            style={{ color: 'var(--rsi-color-text-disabled)', background: 'transparent', border: '1px solid var(--rsi-color-border)' }}
          >
      ...
      <div className="flex-1 flex items-center justify-center px-5 sm:px-8 py-8 sm:py-0">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 sm:gap-16 w-full max-w-[1060px] items-center">
          ...
          <div className="flex flex-col gap-8">
            ...
            <div>
              {([t('landing.tagline1'), t('landing.tagline2'), t('landing.tagline3')] as const).map((tag, i) => (
                <div
                  ...
                  style={{
                    fontWeight: 900,
                    color: i === 0 ? 'var(--rsi-dunkelblau)' : i === 1 ? 'var(--rsi-blau)' : 'var(--rsi-color-text-disabled)',
                  }}
                >
                  {tag}
                </div>
            ...
            {/* Untertitel (Lead) */}
            <p className="text-sm leading-relaxed" style={{ color: 'var(--rsi-color-text-muted)', maxWidth: '420px' }}>
              {t('landing.subtitle')}
            </p>

            {/* Intro — fachlicher Kontext (ISSI / bfu) */}
            <p className="text-[13px] leading-relaxed" style={{ color: 'var(--rsi-color-text-muted)', maxWidth: '460px' }}>
              {t('landing.intro')}
            </p>
            ...
            <div className="flex flex-col gap-4">
              {([
                ...
                <div key={text} className="flex items-start gap-3">
                  <div className="mt-0.5 flex-shrink-0" style={{ color: 'var(--rsi-blau)' }}>{icon}</div>
                  <span className="text-[13px] leading-relaxed" style={{ color: 'var(--rsi-color-text-muted)' }}>{text}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          >
            <h2 className="text-xl font-bold mb-1" style={{ color: 'var(--rsi-color-text)' }}>
              {t('onboarding.welcome')}
            </h2>
            <p className="text-sm mb-7" style={{ color: 'var(--rsi-color-text-muted)' }}>
              {t('onboarding.identifyRanking')}
            </p>

            {/* Name */}
            <label className="block text-[11px] font-bold uppercase tracking-wider mb-1.5" style={{ color: 'var(--rsi-color-text-muted)' }}>
            ...
            <input
              ...
              style={{ padding: '11px 14px', border: nameFehlend ? '1px solid var(--rsi-rot)' : '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-color-text)', fontFamily: 'var(--rsi-font)', marginBottom: nameFehlend ? '4px' : '16px', boxSizing: 'border-box' }}
            />
            {nameFehlend && (
              <p role="alert" aria-live="polite" className="flex items-center gap-1.5 text-xs mb-3" style={{ color: 'var(--rsi-rot)' }}>
            ...
            <select
              ...
              className={inputClass}
              style={{ padding: '11px 14px', border: '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-color-text)', fontFamily: 'var(--rsi-font)', marginBottom: '16px', boxSizing: 'border-box', cursor: 'pointer' }}
            >
              ...
              <div style={{ marginBottom: passwortFehler ? '6px' : '16px' }}>
                ...
                <div className="relative">
                  <input
                    ...
                    className={inputClass}
                    style={{ padding: '11px 44px 11px 14px', border: passwortFehler ? '1px solid var(--rsi-rot)' : '1px solid var(--rsi-color-border)', background: 'var(--rsi-color-bg-secondary)', color: 'var(--rsi-color-text)', fontFamily: 'var(--rsi-font)', boxSizing: 'border-box' }}
                  />
                  <button
                    type="button"
                    ...
                    style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--rsi-color-text-muted)', padding: 0 }}
                  >
            ...
            <button
              ...
              style={{
                padding: '13px',
                background: canStart ? 'var(--rsi-dunkelblau)' : 'var(--rsi-color-bg-tertiary)',
                color: canStart ? 'white' : 'var(--rsi-color-text-disabled)',
                cursor: 'pointer',
                border: 'none', fontFamily: 'var(--rsi-font)',
              }}
            >
            ...
            {/* Datenschutzhinweis */}
            <p className="text-[10px] leading-relaxed mt-4 text-center" style={{ color: 'var(--rsi-color-text-disabled)' }}>
      ...
      {/* ── Footer ── */}
      <div className="flex items-center justify-between gap-3 px-5 sm:px-8 flex-wrap" style={{ minHeight: 'var(--rsi-footer-h)', borderTop: '1px solid var(--rsi-color-border)' }}>
        <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--rsi-color-text-disabled)' }}>
          <span style={{ color: 'var(--rsi-gruen)', fontWeight: 800 }}>●</span>
          {t('landing.systemOnline')} · v{import.meta.env.VITE_APP_VERSION ?? '0.5.0'}
        </div>
        <div className="flex items-center gap-3 flex-wrap">
          <a href={`/impressum.html?lang=${i18n.language}`} target="_blank" rel="noopener noreferrer" className="text-[10px] font-semibold hover:underline" style={{ color: 'var(--rsi-color-text-disabled)' }}>
            {t('landing.linkImpressum')}
          </a>
          <span className="text-[10px]" style={{ color: 'var(--rsi-color-border)' }}>|</span>
          <a href={`/datenschutz.html?lang=${i18n.language}`} target="_blank" rel="noopener noreferrer" className="text-[10px] font-semibold hover:underline" style={{ color: 'var(--rsi-color-text-disabled)' }}>
            {t('landing.linkDatenschutz')}
          </a>
          <span className="text-[10px]" style={{ color: 'var(--rsi-color-border)' }}>|</span>
          <a href={`/glossar.html?lang=${i18n.language}`} target="_blank" rel="noopener noreferrer" className="text-[10px] font-semibold hover:underline" style={{ color: 'var(--rsi-color-text-disabled)' }}>
          ...
          <button
            ...
            style={{ color: 'var(--rsi-color-text-disabled)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
          >
          ...
          <span className="text-[10px]" style={{ color: 'var(--rsi-color-border)' }}>|</span>
          <span className="text-[10px] font-semibold" style={{ color: 'var(--rsi-color-text-disabled)' }}>
          ...
          >
            <div className="flex items-center gap-3 mb-5">
              <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: 'rgba(0,64,124,0.1)' }}>
                <Lock size={18} style={{ color: 'var(--rsi-dunkelblau)' }} />
              </div>
              <div>
                <h3 id="admin-pin-title" className="text-base font-bold" style={{ color: 'var(--rsi-color-text)' }}>{t('admin.pin_titel')}</h3>
                <p className="text-xs" style={{ color: 'var(--rsi-color-text-muted)' }}>{t('admin.pin_hinweis')}</p>
              </div>
            </div>
            ...
            <input
              ...
              style={{
                padding: '14px',
                border: adminPinFehler ? '2px solid var(--rsi-rot)' : '1px solid var(--rsi-color-border)',
            ...
            {adminPinFehler && (
              <p role="alert" aria-live="polite" className="flex items-center justify-center gap-1.5 text-xs mb-3 text-center" style={{ color: 'var(--rsi-rot)' }}>
